...
7. **修完就把规则钉进不变量**：`ARCHITECTURE.md` 的编号不变量列表（**现有 45 条**）+
   `CONTRIBUTING.md` 的约定列表（**现有 36 条**），并在 `CHANGELOG.md` 的 `[Unreleased]` 段登记。
...
**当前基线（第 12 棒完成时实测，提交 `0b571f3`）：**

- `pnpm run verify` 退出 0，**917 个用例全过（98 个测试文件）**，eslint 零 warning
  —— 第 11 棒 911 / 98，第 10 棒 908 / 98，第 9 棒 834 / 89，第 8 棒 727 / 73
...
- `0b571f3` —— 第 12 棒修复切换语言的内容丢失 / 白屏卡死 / 编辑器泄漏（5 处）
