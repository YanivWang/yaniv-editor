...
7. **修完就把规则钉进不变量**：`ARCHITECTURE.md` 的编号不变量列表（**现有 46 条**）+
   `CONTRIBUTING.md` 的约定列表（**现有 37 条**），并在 `CHANGELOG.md` 的 `[Unreleased]` 段登记。
...
- `pnpm run verify` 退出 0，**923 个用例全过（99 个测试文件）**，eslint 零 warning
  —— 第 12 棒 917 / 98，第 11 棒 911 / 98，第 10 棒 908 / 98，第 9 棒 834 / 89
- 覆盖率 Statements 66.61% / Branches 56.16% / Functions 62.84% / Lines 68.24%
- `pnpm run build` 产物预算实测：主 chunk gzip **45575 / 46000**（余量 **425B**）、
  `style.css` **17183 / 19000**、`inline.css` **8953 / 10500**；代码分割断言全过
- `pnpm run test:e2e` **22 passed**
- ⚠️ **主 chunk 余量只有 425B。** 但注意：**注释基本不吃预算**——第 12 棒实测更正了
...
- `2e702b9` —— 第 13 棒选中底色 + 光标色跟随品牌主色，堵上 token 护栏的反引号漏洞
...
### 已知但不修的行为（第 13 棒实测后的结论，别重新纠结）

- **受控推送后撤销按钮会亮着，但点一次没反应。** 量准了：`undoDepth` 在推送后是 1、
  点一次撤销后归 0 且按钮正确变灰；**连续多次推送 `undoDepth` 始终是 1，不累积**；
  推送后用户再编辑，撤销**完全正常**。影响面就是「编辑过→推送→立刻点撤销」这一个序列
  下的**一次**空点击，不循环、不损坏数据。
  这是 prosemirror-history 的固有行为（`addToHistory: false` 的整文档替换会把已有
  历史步骤 rebase 成空，但事件计数还在，而 `can().undo()` 只看计数）。
  所有修法的代价都更大：官方钩子（`tr.setMeta(historyKey, {historyState})`）需要
  未导出的 `HistoryState` / `Branch`；走 tiptap 的 register/unregister 会重建全部
  plugin view（第 10 棒已证明有副作用、打穿过测试）。**风险大于收益，维持现状。**
