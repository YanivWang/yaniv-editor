 ...
 * @description 光标所在块的**行首**输入 `/` 时弹出块类型选择菜单（类似 Notion）。
 * 判定条件是「光标之前的块内文本恰好是 `/` + 若干非空白字符」（见下方 `^\/(\S*)$`），
 * 因此块内光标之后可以已有内容，并不要求整行为空。
 * @features
 * - 监听 / 输入，在光标位置显示命令菜单
 * - 支持输入过滤（如 /h1, /list）
 * - 自动关闭：光标移出匹配范围、删除 `/`、或选区变为非折叠
 *
 * 键盘导航（上/下/回车/ESC）与点击外部关闭由 `BlockPickerMenu` 组件处理，不在本扩展内。
 */
