...
基于 Vue 3 + Tiptap 3 的富文本编辑器组件库（**v0.3.0**）。
...
完整 API 参考与使用指南见 [`docs/`](./docs/index.md)（[在线文档](https://yanivwang.github.io/yaniv-editor/) · 运行 `pnpm docs:dev` 启动本地文档站）。[在线 Demo](https://yanivwang.github.io/yaniv-editor/examples/)。破坏性变更与迁移说明：[`CHANGELOG.md`](./CHANGELOG.md)。架构设计：[`ARCHITECTURE.md`](./ARCHITECTURE.md)。
...
| `uploadImage`            | `(file: File) => Promise<string>`             | 回落到 `blob:` URL | 图片上传处理函数                              |
| `uploadVideo`            | `(file: File) => Promise<string>`             | 回落到 `blob:` URL | 视频上传处理函数                              |
...
`notion` 主打块级编辑（斜杠菜单 + 拖拽手柄），并启用图片、视频、表格、数学公式、大纲、查找替换、Office 粘贴和 AI；格式刷默认关闭。其交互以浮动菜单 / 块菜单为主，**不展示**固定顶部工具栏与底部栏。

隐藏顶栏只是去掉了这两个**按钮**，能力本身在 `notion` 下依然可用：大纲面板在正文旁有自己的
展开把手（`.outline-rail__handle`），查找替换面板与 Ctrl/Cmd+F 快捷键挂在 `searchReplace` gate 上、
不依赖顶栏。
