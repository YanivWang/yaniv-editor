...
`notion` 开启块编辑（斜杠命令、拖拽手柄），以及图片、视频、表格、公式、大纲、查找替换、Office 粘贴和 AI（格式刷关闭）。隐藏固定顶部工具栏和页脚，依赖浮动/块级交互。

注意：`notion` 隐藏顶栏后，大纲开关与查找替换**按钮**不存在（两者只挂在顶栏），但能力本身仍可用——大纲用正文旁的 rail 展开把手，查找替换用 Ctrl/Cmd+F。详见 [大纲目录](../features/outline.md) 与 [查找替换](../features/find-replace.md)。
