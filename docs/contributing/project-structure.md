...
关键约束只有一条：**`content.css` 必须排在 `appearance/styles/*.css` 之前**——外观层只改 token 与排版，不能被结构层反向覆盖。只有 `index.css` 在末尾导入 `appearance/styles/*.css`；`inline.css` **不含任何 appearance 文件**（Inline 无 `appearance` prop），因此那条排序约束对它不适用。
