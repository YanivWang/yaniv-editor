...
- `EditorShell` 在根内渲染 `.yaniv-editor__overlay-portal`；bubble menu、BlockPicker、mention、AI popover、Ant Design 浮层、自建 Toast/Notice 等均挂载于此，**不**使用 `document.body`（drag preview / 隐藏 file input 除外）；**禁止** Ant Design 静态 `message` / `notification`。
