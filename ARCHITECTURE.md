...
Vue 3 + Tiptap 3 富文本编辑器库的分层架构（对应 v0.3.0）。
...
15. **禁止模块级可变状态** — 库需支持同页多实例，`let x = ...` 形式的模块级配置会让实例互相覆盖。所有实例相关状态走 provide/inject 或 **owner 键控注册表**（每实例一个 `Symbol`）。历史事故：outline `scrollParent`、AI `hostConfig`（未传 `ai-config` 的实例会静默复用另一实例的密钥）、`aiSuggestionManager` 的构建期 `bindLocale`。无 owner 的查询在存在多个登记方时必须**显式返回 null 并告警**，不得任选其一——任选其一正是缺陷本身。**例外清单以静态护栏 `src/moduleLevelState.test.ts` 为准**：那里逐条登记了当前 12 处模块级可变状态（`locales/manager` 的语言包缓存与全局默认、`translateStore` 的翻译目标语言、`features/ai/config` 的用户 AI 配置）以及**它们为什么不该按实例隔离**；新增一处未登记的就会转红，清单里留下已删掉的条目同样转红。
...
42. **定义了的 `--ye-*` token 必须有人 `var()` 引用** — 零消费方的 token 不报错、
    ...
    **配了值却没写规则**（`--ye-border-focus` 三套外观各配了色而编辑区有意 `outline: none`；
    ...
    ⚠️ **消费方只认代码里的引用，注释与测试都不算。** 「JS 字符串读写」那一条的正则
    带反引号（``/["'`](--ye-[\w-]+)["'`]/``），而本仓库的中文注释通行写法正是
    Markdown 风格的 `` `--ye-x` ``——注释里提一句就被当成了消费方。第 13 棒因此
    漏掉了 `--ye-caret`：它按形状 C 在三处分层声明好了，全仓却没有任何规则读它，
    而 `variables.css` 的一段注释里恰好写着 `` `--ye-caret` ``。
    测试文件同理：断言正则里出现一次 `var(--ye-x)` 就能让死 token 永久免检。
    扫描器现已先掩注释、并整体跳过 `*.test.ts`。
...
46. **文本编辑的视觉反馈（选中底色、光标色）必须跟着外观的品牌主色走** —
    ...
    RGB 恒等于同一作用域的 `--ye-primary`，只有透明度分档（选中底色亮 30% / 暗 40%，
    深色底上同样的半透明色看起来更弱，要给得更足）。
    写**字面值**而不是 `color-mix(… var(--ye-primary) …)`：构建 target 是
    chrome105 / safari16 / firefox110，都低于 color-mix 的支持线。
    代价是与主色脱钩——改了主色忘了改选中色不会报错，只会露出一抹旧品牌色，
    这份「本该由 var() 保证的一致性」由 `styles/selectionColor.test.ts` 补上。
    `::selection` **只设 `background-color`、不设 `color`**：压掉文字色会把代码块的
    语法高亮、链接色、AI 高亮一起抹平。浏览器实测六种组合的选中文字对比度
    7.08~~13.92，全部远超 WCAG AA 的 4.5；真正卡住取值的是「选中背景 vs 正常背景」
    的可辨识度（30/40% 下为 1.45~~1.97）。
    `::selection` 不继承，必须同时写根节点自身与后代两条选择器；
...
50. **改块类型要搬运行内内容，不能按 `textContent` 重建** — 用文字重建会把 mark
    （加粗、斜体、链接的 `href`）、行内节点（换行、mention）全部丢掉，多块源
    （列表、表格、引用）还会把各块的文字粘成一段。正确做法是取源块的行内 `Fragment`
    按源块切分，逐个包进目标块类型。两个例外要单独处理：
    - `codeBlock` 的 content 是 `text*`、`marks: ""`，只能收纯文本，换行用 `\n`；
    - 反过来，从 `codeBlock` 出来的文本里带字面 `\n`，搬进段落/标题前必须换成
      `hardBreak`——实测 `<p>a\nb</p>` 经 `getHTML()` → `setContent()` 往返一次
      就变成 `<p>a b</p>`，换行**永久丢失**（HTML 解析把换行当普通空白折叠）。

51. **指针落在自己弹出的浮层上时，不得按指针坐标重新做元素命中** — 浮层往往比
    触发它的元素高一个数量级（实测：段落 26px、块菜单 139px、块选择器 340px），
    按指针 Y 去探内容会命中浮层**下方**的元素，于是「鼠标刚移向菜单，菜单就没了」。
    判定「是不是自己的浮层」要把这条链上的每一层都算进去——触发器、菜单、菜单唤起的
    二级面板都算。`DragHandleExtension` 的 `isPointerOverOwnSurfaces` 是这条的实现，
    `mousemove` / `mouseleave` / `mousedown` 三条路径共用同一个判据。

52. **挂到 `document.body` 的临时 DOM，必须由创建者持有引用并在销毁时收回** —
    它不在编辑器根节点内，编辑器卸载不会带走它。用 `document.querySelectorAll(...)`
    全局清理有两个毛病：销毁路径上没人调用时会永久泄漏，而同页多实例时又会误删
    别的实例正在用的那一份。实测 `.drag-handle__drag-image`（整个块的深拷贝）两种
    都中了。持引用后 `dragend` / 停止交互 / `destroy()` 三处收敛到同一个 `remove`。

53. **共享单例里的句柄，结束时要按身份清，不能无条件置空** — `aiSuggestionManager`
    只存一个 `AbortController`。换流的时序是：新流设句柄 → 旧流被 abort → 旧流的
    fetch 以 `AbortError` 走 `onError` 并在那里清句柄，而此刻句柄已经是**新流**的。
    无条件 `setAbortController(null)` 等于把新流的取消能力一起扔掉：用户点「取消」
    毫无反应，新流继续消耗配额、继续往同一个单例写建议文本。
    正确写法是 `clearAbortController(自己那个)`（`this.abortController === 传入的`
    才清空）。已立护栏禁止 `setAbortController(null)` 出现在源码里。

54. **持久化用户选择要存标识符，不要存界面文案** — 显示名随 locale 变，标识符不变。
    翻译目标语言曾经存的是标签（如「英语」），切换编辑器语言后按钮显示成
    「Translate to 英语」、菜单里的选中标记也反查不到，而且这份配置是持久化的，
    错乱会一直跟着用户走。判据：**这个值是拿来"记住用户选了什么"的，还是拿来"给人看"的**
    ——前者一律存 code，显示时再 `t()`。

55. **失败路径必须有用户可见的反馈，而且要覆盖每一条** — 媒体上传把
    `show-upload-list` 关掉之后，antd 把文件标成 error 用户根本看不见，`catch` 里
    只有 `onError?.(e)` 就等于完全无声：弹窗照常关闭、内容没插入、界面上什么也没有。
    `messages.imageUploadFailed` 这条文案**早就写好、零消费**——文案存在而无人使用，
    是「有人要了但没接上」的信号（不变量 26 的同型）。
    ⚠️ 同一个返回值不要同时表示「用户取消」和「操作失败」：调用方分不开，
    失败就会被当成取消静默掉。分不开时，提示要放在最靠近失败点、还知道原因的地方。

56. **组件卸载时要清掉自己排的定时器，判据看「这一个」而不是「总数」** —
    关菜单时清、卸载时不清，是同一个清理只做了一半（`ToolbarDropdownButton` 的
    子菜单延时收起）。⚠️ 验证时不能断言 `vi.getTimerCount()` 的**总数**下降——
    卸载时 antd 与 Vue 自己也会清一批，总数无论如何都会掉，没写清理照样绿。
    盯组件排的那一个（按它独有的延时值认出来），看它的 id 有没有被 `clearTimeout`。

57. **可访问名承诺了什么，键盘就要能做到什么** — `MathNodeView` 的公式是个
    `<button aria-label="编辑公式">`，而键盘激活它只会选中节点：真正的编辑入口
    只有 `dblclick`，**双击没有键盘等价物**。空公式的占位文案也一直写着
    「点击编辑公式」，而单击同样不进编辑。补键盘路径（Enter / Space）比改文案彻底：
    改文案只是不再说谎，键盘用户依然没有入口。
